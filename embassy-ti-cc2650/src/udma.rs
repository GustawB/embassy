...
mod internals {
    ...
    pub(super) struct Udma(*const pac::udma0::RegisterBlock);
    unsafe impl Send for Udma {}
    unsafe impl Sync for Udma {}

    const UDMA_REGISTER_BLOCK_ADDR: usize = 1073872896;
    pub(crate) static LOCAL_UDMA: Udma = Udma(UDMA_REGISTER_BLOCK_ADDR as *const _);

    impl Deref for Udma {
        type Target = pac::udma0::RegisterBlock;
...
impl Udma {
    ...
    pub(crate) fn enable(&self) {
        ...
        LOCAL_UDMA.ctrl.write(|w| unsafe { w.bits(map_addr) });

        LOCAL_UDMA.cfg.write(|w| w.masterenable().set_bit());
    }
...
mod channel_control_entry_kind {
    ...
    #[cfg(feature = "full_LOCAL_UDMA_table")]
...
impl<const INDEX: u32> ChannelControlEntry<Primary, INDEX> {
    fn enable(&self) {
        ...
        LOCAL_UDMA.setchannelen.write(|w| unsafe { w.chnls().bits(1 << INDEX) })
    }
    ...
    fn is_enabled(&self) -> bool {
        LOCAL_UDMA.setchannelen.read().chnls().bits() & (1 << INDEX) != 0
    }

    #[allow(unused)]
    fn software_request(&self) {
        LOCAL_UDMA.softreq.write(|w| unsafe { w.chnls().bits(1 << INDEX) })
    }

    fn request_done(&self) -> bool {
        LOCAL_UDMA.reqdone.read().chnls().bits() & (1 << INDEX) != 0
    }

    fn request_done_clear(&self) {
        LOCAL_UDMA.reqdone.write(|w| unsafe { w.chnls().bits(1 << INDEX) })
    }
}
